...
class DIV2K:
    ...
    def __init__(self,
        ...
        self.caches_dir = os.path.join(data_dir, 'caches/')
    ...
    def dataset(self, batch_size=16, repeat_count=None, rand_crop=True, rand_transform=False, rand_jpeg=False, cache=True):
        ...
        if rand_crop:
            ds = ds.map(lambda lr, hr: random_crop(lr, hr, scale=self.scale, hr_crop_size=self.resolution), 
        ...
        if rand_transform:    
            ...
            if rand_jpeg:
                ds = ds.map(lambda lr, hr: random_jpeg(lr, hr, quality=self.quality), num_parallel_calls=AUTOTUNE)
...
def random_crop(lr_img, hr_img, hr_crop_size=(96,96), scale=4):
...
def random_jpeg(lr_img, hr_img, quality):
    
    rn = tf.random.uniform(shape=(), maxval=1)
    return tf.cond(rn < 0.1,
                   lambda: (tf.io.decode_jpeg(tf.io.encode_jpeg(lr_img, quality=quality)), hr_img),
                   lambda: (lr_img, hr_img))

def random_saturation(lr_img, hr_img):
    
    rn = tf.random.uniform(shape=(), maxval=1)
    return tf.cond(rn < 0.1,
                   lambda: (tf.image.rgb_to_grayscale(lr_img), tf.image.rgb_to_grayscale(hr_img)),
                   lambda: (lr_img, hr_img))
